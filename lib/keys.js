...
var dateForKey = function(dimension, ts, delta) {
...
var keys = module.exports = {
  all: { // a
    delta: 0,
    key: function () { return 'a';},
    format: function () {
      return 'all';
    }
  },
  year: {
    key: function (ts, delta) { // 15
      var date = new Date(ts);
      if (delta !== undefined) date.setUTCFullYear(date.getUTCFullYear() + delta);
  ...
  month: {
    key: function (ts, delta) { // 1502
      var date = new Date(ts);
      if (delta !== undefined) date.setUTCMonth(date.getUTCMonth() + delta);
  ...
  day: {
    key: function (ts, delta) { // 150205
      var date = new Date(ts);
      if (delta !== undefined) date.setUTCDate(date.getUTCDate() + delta);
  ...
  weekday: {
    key: function (ts, delta) { // 6 finite
      var date = new Date(ts);
      if (delta !== undefined) date.setUTCDate(date.getUTCDate() + delta);
  ...
  schedule: {
    delta: 3600000,
    key: function (ts, delta) { //609 finite
      var date = new Date(ts);
      if (delta !== undefined) date.setUTCHours(date.getUTCHours() + delta);
